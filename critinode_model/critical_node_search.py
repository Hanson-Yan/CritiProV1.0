...
def normalize_scores(scores: Dict[str, float]) -> Dict[str, float]:
    ...
    total = values.sum()
    if total == 0:
        raise ValueError("所有节点分数为零，无法归一化。")
    normalized = values / total
    return dict(zip(scores.keys(), normalized))


def cumulative_threshold_selection(scores: Dict[str, float], p: float = 0.3) -> list:
    """
    累积分度阈值法 (CDF)：选取最小K使累计贡献 >= p。
    输入: scores = {node: score}，已经是归一化份额。
    """
    sorted_nodes = sorted(scores.items(), key=lambda x: x[1], reverse=True)
    cum_sum = 0.0
    selected = []
    for node, val in sorted_nodes:
        selected.append((node, val))
        cum_sum += val
        if cum_sum >= p:
            break
    return selected
...
def identify_key_nodes_adaptive(file_path, p: float = 0.3):
    """
    自适应识别关键节点 (CDF)。
    """
    # 读取数据
    ...
    # 累积分度阈值法
    key_nodes = cumulative_threshold_selection(normalized_scores, p=p)
